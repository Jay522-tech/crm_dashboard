            ...
            <div className="grid grid-cols-1 xl:grid-cols-3 gap-4 min-h-0">
                <div className="xl:col-span-2 grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div className="rounded-xl border border-border bg-card p-4 shadow-sm flex flex-col">
                        <div className="flex items-center justify-between mb-3">
                            <h3 className="text-sm font-semibold text-foreground flex items-center gap-2">
                                <FileText size={16} className="text-muted-foreground" /> Templates
                            </h3>
                        ...
                        <div className="space-y-2 overflow-y-auto max-h-[12rem] pr-1">
                            {templates.length > 0 ? templates.map((t) => (
                                <div key={t._id} className="p-2.5 rounded-lg border border-border/70 bg-muted/30 hover:border-primary/30 transition-colors cursor-pointer group">
                                    ...
                                    <p className="text-[11px] text-muted-foreground line-clamp-1 mt-0.5">{t.subject || t.content}</p>
                                </div>
                            )) : (
                                <p className="text-xs text-muted-foreground/70 py-4 text-center italic">No templates created yet</p>
                            )}
                        </div>
                    </div>
                    ...
                    <div className="rounded-xl border border-border bg-card p-4 shadow-sm flex flex-col">
                        <div className="flex items-center justify-between mb-3">
                            <h3 className="text-sm font-semibold text-foreground flex items-center gap-2">
                                <Inbox size={16} className="text-muted-foreground" /> Stats
                            </h3>
                        </div>
                    ...
                    <div className="md:col-span-2 rounded-xl border border-border bg-card p-4 shadow-sm min-h-[22rem] flex flex-col">
                        <div className="flex items-center justify-between mb-4">
                            <p className="text-sm font-semibold text-foreground">Recent Communications</p>
                            ...
                            {communicationHistory.length > 0 ? (
                                <div className="space-y-3">
                                    {communicationHistory.map((log) => (
                                        <div key={log._id} className="flex items-start gap-3 p-3 rounded-xl border border-border/50 hover:bg-muted/30 transition-colors">
                                            ...
                                            <div className="min-w-0 flex-1">
                                                ...
                                                <p className="text-xs text-muted-foreground line-clamp-1 mt-0.5">{log.content}</p>
                                                <div className="flex items-center gap-2 mt-2">
                                                    ...
                                                    <span className="text-[10px] text-muted-foreground">• By {log.sentBy?.name || 'System'}</span>
                                                </div>
                                            </div>
                                        </div>
                                    ))}
                ...
                <div className="space-y-4">
                    <div className="rounded-xl border border-border bg-card p-4 shadow-sm">
                        <p className="text-sm font-semibold text-foreground mb-1">Integrations</p>
                        <p className="text-xs text-muted-foreground mb-4">Connect your message providers.</p>
                        <div className="space-y-2">
                            {[
                                ...
                                <div key={integ.name} className="flex items-center justify-between p-3 rounded-xl border border-border/70 bg-muted/30 group hover:border-primary/20 transition-colors">
                                    <span className="text-sm text-foreground font-medium">{integ.name}</span>
                                    <span className="text-[10px] font-bold bg-card border border-border text-muted-foreground rounded-full px-2 py-0.5 uppercase">
                                        {integ.status}
                                    </span>
                                </div>
                            ))}
